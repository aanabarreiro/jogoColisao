...
document.addEventListener('keydown', (event) => {
    ...
    let corAtual = window.getComputedStyle(movable).backgroundColor;

    if (colisaoDetectada(movable, target)) {
        alert('Colisão detectada!');

        if (corAtual === 'rgb(0, 128, 0)') {
            movable.style.backgroundColor = 'blue';
            alert('Cor alterada para azul');
        } else if (corAtual === 'rgb(0, 0, 255)') {
            movable.style.backgroundColor = 'green';
            alert('Cor alterada para verde');
        }
    }
});
